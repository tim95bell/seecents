...
class LedgerEntryTest {
    ...
    inner class Create {
        ...
        fun `can have equal createdAt and effectiveAt in an entry`() {
            testEntry(createdAt = TEST_INSTANT[0], effectiveAt = TEST_INSTANT[0]).assertRight()
        }

        @Test
        fun `can have createdAt greater than effectiveAt in an entry`() {
            testEntry(createdAt = TEST_INSTANT[1], effectiveAt = TEST_INSTANT[0]).assertRight()
        }

        @Test
        fun `can NOT have createdAt smaller than effectiveAt in an entry`() {
            testEntry(createdAt = TEST_INSTANT[0], effectiveAt = TEST_INSTANT[1])
        ...
        fun `expense entries can contain lines where fromId equals toId`() {
            testLine(from = TEST_USER_ID[0], to = TEST_USER_ID[0]).flatMap { line ->
        ...
        fun `expense entries can contain lines where fromId NOT equals toId`() {
            testLine(from = TEST_USER_ID[0], to = TEST_USER_ID[1]).flatMap { line ->
                testEntry(type = LedgerEntryType.Expense, lines = NonEmptyList.of(line))
            }.assertRight()
        }
        ...
        fun `can NOT have creatorId that is not in group`() {
            testEntry(creatorId = TEST_USER_ID[2]).assertLeftEq(LedgerEntry.CreateError.CreatorNotInGroupError)
        }

        @Test
        fun `can NOT have line fromId that is not in group`() {
            testEntry(lines = NonEmptyList.of(testLine(from = TEST_USER_ID[2]).assertRight().value))
                .assertLeftEq(LedgerEntry.CreateError.LineUserNotInGroupError)
        }

        @Test
        fun `can NOT have line toId that is not in group`() {
            testEntry(lines = NonEmptyList.of(testLine(to = TEST_USER_ID[2]).assertRight().value))
                .assertLeftEq(LedgerEntry.CreateError.LineUserNotInGroupError)
        }
    }
}
